...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    private Entry<K, V> getEntryIfAvailable(Object key) {
        Reference<K, V> ref = getReference(key, true);
     ...
     * @param restructureIfNecessary restructure if necessary
     * @return the reference, or {@code null} if not found
     */
    
    protected final Reference<K, V> getReference( Object key, boolean restructureIfNecessary) {
        int hash = getHash(key);
        return getSegmentForHash(hash).getReference(key, hash, restructureIfNecessary);
    }
    ...
    private V put( final K key,  final V value, final boolean onlyIfAbsent) {
        return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
            ...
            protected V execute(Reference<K, V> ref, Entry<K, V> entry, Entries entries) {
                ...
                if (entries == null) {
                    throw new NullPointerException("No entries segment");
                }
                entries.add(value);
                return null;
            }
    ...
    public void purgeUnreferencedEntries() {
        for (Segment segment : this.segments) {
            segment.restructureMapIfNecessary(false);
        }
    }
    ...
    protected final class Segment extends ReentrantLock {
        ...
        public Reference<K, V> getReference( Object key, int hash, boolean restructureIfNecessary) {
            if (restructureIfNecessary) {
                restructureMapIfNecessary(false);
            }
            if (this.count == 0) {
                return null;
            }
        ...
        public <T> T doTask(final int hash,  final Object key, final Task<T> task) {
            ...
            if (task.hasOption(TaskOption.RESTRUCTURE_BEFORE)) {
                restructureMapIfNecessary(resize);
            }
            ...
            try {
                ...
                Entries entries = new Entries() {
                    ...
                    public void add( V value) {
                        @SuppressWarnings("unchecked")
                        Entry<K, V> newEntry = new MapEntry<>((K) key, value);
         ...
         */
        protected final void restructureMapIfNecessary(boolean allowResize) {
    ...
    protected static final class MapEntry<K, V> implements Map.Entry<K, V> {
        ...
        public String toString() {
            ...
            int kLen;
            int vLen;
            char[] chars = new char[(kLen = (k = String.valueOf(key)).length()) + (vLen = (v = String.valueOf(val)).length()) + 1];
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains( Object o) {
            if (o instanceof Map.Entry<?, ?>) {
                ...
                Reference<K, V> ref = ConcurrentReferenceHashMap.this.getReference(entry.getKey(), false);
